...
-- 从 star 过仓库的开发者中找到 有过 fork 仓库行为的人
   ...
   , repo_id_who_be_stared as (SELECT distinct repo_id
                               FROM actor_star_vscode
                               WHERE rn <= ${num_people_per_repo})
   , actor_pr_repo as (select actor_id, repo_id, concat(toString(repo_id), '#', toString(issue_number)) as pr_id
                       from events
                       where platform = 'GitHub'
...
SELECT actor_id, repo_id, pr_id, COUNT(*) as weight
FROM actor_pr_repo
GROUP BY actor_id, repo_id, pr_id;
